...
def _test_create_dates_for_one_year(db: Session) -> None:
    ...
    dates = crud.get_dates(db, limit=1000)
    ...
    assert crud.get_date(db, day_id=day.id, year=DAY_PASKHA.year).movable_day_id == 1
    assert crud.get_date(db, day_id=crud.get_day(db, month=8, day=31).id, year=DAY_PASKHA.year)
    assert crud.get_date(db, day_id=crud.get_day(db, month=9, day=1).id, year=DAY_PASKHA.year + 1)
    assert crud.get_date(db, day_id=crud.get_day(db, month=12, day=31).id, year=DAY_PASKHA.year + 1)
    assert crud.get_date(db, day_id=crud.get_day(db, month=1, day=1).id, year=DAY_PASKHA.year + 1)
...
def _test_create_dates_for_years(db: Session) -> None:
    ...
    for day in (date(2031, 4, 16) + timedelta(days=num_day) for num_day in range(365)):
        ...
        if 9 <= day.month <= 12:
            year += 1
        assert crud.get_date(db, day_id=crud.get_day(db, month=day.month, day=day.day).id, year=year)
