...
def _lls_bookmarks_main(pdf_path: Path, *, out_pdf_path: Path, lls_book_rus: LlsBookFullType):
    reader = PdfReader(pdf_path)
    writer: PdfWriter = reader2writer(reader)
    bookmarks: list[PdfBookmark] = []
    lls_bookmarks: list[PdfBookmark] = get_lls_bookmarks(lls_book_rus)
    for i, lls_bookmark in enumerate(lls_bookmarks[:-1]):
        ...
        if (next_lls_bookmark := lls_bookmarks[i + 1]).start_on_new_page:
            end_bookmark = PdfBookmark(title='-', page=next_lls_bookmark.page_num - 1)
    ...
    end_lls_bookmark: PdfBookmark = lls_bookmarks[-1]
    bookmarks.append(
        PdfBookmark(title=end_lls_bookmark.title, page_num=end_lls_bookmark.page_num, color=end_lls_bookmark.color)
    )
...
def main5():
    ...
    part_1_bookmarks: list[PdfBookmark] = get_bookmarks(part_1_reader)
    part_1_bookmarks: list[PdfBookmark] = part_1_bookmarks[-1:]
...
def _test_add_root_bookmark_and_bookmarks():
    ...
    writer: PdfWriter = reader2writer(reader)
    bookmarks: list[PdfBookmark] = get_bookmarks(reader)
    root_bookmark = PdfBookmark(title='Евангелие от Иоанна', page_num=bookmarks[0].page_num)
...
def lls_bookmarks_roman_main(lls_book: LlsBookFullType, lls_book_num: int) -> None:
