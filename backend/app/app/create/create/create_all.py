...
def create_all_holidays(db: Session, *, session: requests.Session) -> None:
...
def main(db: Session, *, session: requests.Session) -> None:
    # create_all_holidays(db, session=session)
    create.update_saints(db, session=session)
