...
def create_holiday(db: Session, *, holiday_data_in: schemas.HolidayDataCreate) -> models.Holiday:
...
def create_movable_saint_holiday(
    ...
    saint = crud.saint.get_by_slug(db, slug=movable_saint_holiday_in.saint_in.slug)
    if not saint:
        saint = crud.saint.create(db, obj_in=movable_saint_holiday_in.saint_in)
