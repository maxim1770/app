...
def read_weeks(cycle_num: int, db: Session = Depends(deps.get_db)):
    weeks: list[models.Week] = crud.get_weeks(db, cycle_num=cycle_num)
    return weeks


@router.get('/{sunday_num}', response_model=schemas.Week)
def read_week(cycle_num: int, sunday_num: int, db: Session = Depends(deps.get_db)):
...
def create_week(cycle_num: int, week: schemas.WeekCreate, db: Session = Depends(deps.get_db)
                ):
